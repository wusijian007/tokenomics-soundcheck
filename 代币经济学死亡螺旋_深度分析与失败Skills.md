...
> **v2 可操作版**:skill pack 中的 [`scorecard.md`](skills/tokenomics-death-spiral-audit/references/scorecard.md) 已升级为 12 行(含 #11 #12)、满分 54 的可测量版本——每行给出公式、数据源与 0/1/2 判定阈值,并在 18 个历史案例(10 崩盘 + 8 压力幸存者)上完成回测校准:崩盘组 12–37 分,幸存组 1–11 分,无幸存者触发引擎红线(见 `data/scorecard_calibration.py` 与分离度图)。此外已做 **out-of-sample 验证**:15 个泄漏审计的留出案例上判定规则 15/15 正确,并有冻结的前瞻性登记册预注册可证伪预测(见 [`validation/`](validation/README.md))。配套的标准化审计流程见 [`audit-protocol.md`](skills/tokenomics-death-spiral-audit/references/audit-protocol.md),幸存者对照组分析见 [`survivors.md`](skills/tokenomics-death-spiral-audit/references/survivors.md)。
>
> **v4 增补**:除 12 行螺旋分外,新增独立的**安全面板**(S13/S14/S15,腐化成本账本,见 §5b 与 [`economic-security.md`](skills/tokenomics-death-spiral-audit/references/economic-security.md)),审计协议加入了**估值模块**(货币观/股权观分 regime + 风险×估值四象限)与**盲点登记册**,并给出 λ 的雅可比谱半径形式化与反身性 beta 实测方案([`lambda-formalization.md`](skills/tokenomics-death-spiral-audit/references/lambda-formalization.md))。
...
> 这些公理的正向展开——含完整设计流程、参数基准表与上线清单——见 skill pack 中的 [`design-playbook.md`](skills/tokenomics-death-spiral-audit/references/design-playbook.md)。
