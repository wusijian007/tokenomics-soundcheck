...
下面四张图由 `simulations/` 下的 Python 模型生成(`python run_all.py`),每个模型校准自真实崩盘参数,用来**可视化相变本身**——证明崩溃是机制内生(λ>1)、从现实参数即可达到,而非外生意外。完整说明见 [skills/.../references/simulations.md](skills/tokenomics-soundcheck/references/simulations.md)。
...
**评分卡校准(18 案例:10 崩盘 vs 8 压力幸存者,见 [data/scorecard_calibration.py](data/scorecard_calibration.py))**
![scorecard separation](simulations/charts/data_scorecard_separation.png)
崩盘组在 v2 评分卡上得 12–37 分,幸存组 1–11 分;没有幸存者触发引擎红线。边界上的一对(ICP 12 分崩盘,CRV 11 分幸存)恰好由"归零测试"分辨——正是判定规则的第二级。

**Out-of-sample 留出集(15 个泄漏审计、从未参与工具构建的案例,见 [validation/](validation/README.md))**
![holdout separation](simulations/charts/data_holdout_separation.png)
样本外的原始总分在"中危"区间重叠——但"引擎 → 结构 → 锚"三级判定规则对 15 个留出案例 15/15 全部分类正确(6 个引擎螺旋、2 个结构阴跌、7 个压力幸存者)。另有冻结的[前瞻性登记册](validation/prospective-registry.md)对在世项目预注册可证伪预测,2027/2028 复核——无后见之明偏差的金标准层。
...
- **红旗**:协议代币互为抵押;少数关联做市商提供主要流动性;跨协议同源风险敞口;**关键人杠杆**(创始人/巨鲸用治理代币抵押巨额借贷,CRV 2023.8)。
- **实例**:Terra → 3AC → Celsius/Voyager → FTX 连锁。
- **解药**:抵押品多元化 + 相关性压力测试;限制关联方做市集中度;隔离风险敞口。

### Skill #11 — 雇佣兵积分 / 租来的 TVL(2023–26 增补)
- **内核**:增长指标(TVL/用户/交易量)是用"积分 → 空投"的未来代币支出**租来的**;需求本身就是对代币的远期索取权,代币一发,需求同时蒸发。
- **结构**:积分计划 = 远期出售的增发。空投预期价值随 FDV 预期缩放 → 反身。所有农民共享同一份快照/TGE 日历 → 退出是**同步的**:空投解锁(供给冲击)与雇佣兵资本撤离(需求悬崖)落在同一天。模型 4(供给过剩)+ 模型 2(新钱博弈)的组合。
- **红旗**:多数 TVL 在积分公告后才进场;每次快照后资金撤离;单位 TVL 手续费收入远低于同行(资金只停泊不使用);多季积分层层加码;积分被加杠杆耕作(与 #12 叠加)。
- **实例**:Blast(2024 年中空投后 TVL 悬崖)、friend.tech(奖励周期结束后活跃与费用崩塌)、2024 年 LRT 积分潮。
- **解药**:奖励在 TGE 后归属/锁定;奖励**流量**(真实付费)而非**存量**(停泊的 TVL);公开"有机基线"面板(若今天停发积分还剩多少);空投规模与 TGE 流通比例设上限。

### Skill #12 — 递归杠杆循环(2022–26 增补)
- **内核**:把生息衍生品循环抵押去借它自己的底层资产(存 stETH → 借 ETH → 再买 stETH → 循环),或给"稳定"套息收益加杠杆(UST Degenbox、资金费率套息)。底层资产可以是健康的——循环本身给**系统**加装了清算通道,使系统反身。
- **结构**:循环 LTV 为 `ℓ` 时系统杠杆 → `1/(1−ℓ)`。小幅折价 → 触发密集清算线 → 强平把衍生品砸进稀薄流动性 → 折价加深 → 更多清算。λ>1 经由清算通道成立。**临界条件:潜在平仓规模 > 真实二级市场深度。**
- **红旗**:衍生品流通量大头躺在杠杆循环里;循环 APY ≫ 基础 APY;清算线密集分布在窄区间;DEX 深度相对循环规模稀薄;依赖资金费率的收益被宣传成"稳定";相关抵押品无 LTV/供应上限。
- **实例**:2022.6 stETH 循环平仓潮(放大了 Celsius/3AC;stETH 本身守住了——见 skill pack `survivors.md`)、UST Degenbox(Abracadabra)、2024.4 ezETH 脱锚清算级联。观察项:负资金费率长周期下的杠杆套息稳定币。
- **解药**:对自引用/强相关抵押品设 LTV 与供应上限;用**真实 DEX 深度**(而非 TVL)压测全量平仓;预言机偏离熔断;绝不把套息收益宣传成本金稳定。

---

## 5b. 经济攻击轴(Skill #13–#15)——代码没错、机制被定价错

Skill #1–#12 描述的是*反身性动力学*(系统放大自身下跌)。还有一条正交的失败轴——**离散经济攻击**:合约分毫不差地执行,被低估的是机制本身。Beanstalk、Mango、Steem 收购都不是黑客,而是*购买*。统一工具是**腐化成本账本**:对每个控制面算 `利润 = 可提取价值 − 腐化成本`,在对抗性融资(闪电贷、借来的质押、租来的投票)下只要为正,就把该控制面当作已被攻破来定价。这是 Budish 共识安全下界向治理/预言机/流动性的推广。完整框架、实例与解药见 [economic-security.md](skills/tokenomics-soundcheck/references/economic-security.md)。

### Skill #13 — 可捕获治理(攻击面)
- **内核**:凑齐决定性法定人数的成本 < 该法定人数掌控的价值(国库、增发权、排放键)。
- **结构**:`成本(法定人数) < 国库 + NPV(增发控制权)`。可闪电贷的投票 + 无时间锁 → 左边趋近 0,任何国库都成了摆着的悬赏。
- **红旗**:投票在取得的同一笔交易内即可用;无/可绕过时间锁;法定人数相对可借流通量很小;国库用原生代币(抬高奖池);单票贿赂租金 ≪ 币价;托管质押可投票。
- **实例**:Beanstalk(2022.4,闪电贷超级多数,≈$180M)、Build Finance(2022.2)、Tornado 治理(2023.5)、Steem(2020);已防御:Curve vs Mochi(2021.11)。
- **解药**:投票锁定 + 足够长的时间锁(把腐化成本从"手续费"变成"扛过你引发的崩盘的持仓风险");法定人数下限按可借流通量设定;国库/增发操作设更高门槛;窄而可问责的紧急否决;国库不用原生代币。仿真:`sim6_governance_capture.py`(7 天时间锁把约 84% 的可攻击平面翻成安全)。

### Skill #14 — 可操纵预言机杠杆(攻击面)
- **内核**:某杠杆场所接受一个"移动成本 < 它释放的信用"的价格作抵押或结算。
- **结构**:`成本(在预言机窗口内移动 X%) < 按虚高标记可借出的价值`。成本随预言机取价场所的真实深度缩放,价值随 LTV × 上限缩放。
- **红旗**:薄资产用现货/单一场所预言机;无供应/借贷上限;LTV 按资产类别而非操纵成本设定;预言机窗口过短;自身代币按自家薄市标记作抵押;关键人级仓位相对深度过大。
- **实例**:Mango(2022.10,≈$114M)、Venus/XVS(2021)、Inverse Finance(2022)、Moola(2022)。
- **解药**:LTV/上限作为操纵成本的函数;抗操纵的多场所预言机 + 偏离熔断;拒绝为易被移动的资产上杠杆;未平仓上限绑定现货深度。

### Skill #15 — 供给侧补贴错配(DePIN / 工作网络)
- **内核**:排放为*产能*(硬件/存储/覆盖)买单,而付费需求从未到来——一台租着基础设施剧场的排放引擎。#3 的职业化版本。
- **结构**:`服务收入 / 排放价值 ≪ 1` 且长期如此。sink 真实但极小;代币持有者补贴没人租用的供给。
- **红旗**:产能指标增长而付费需求持平;收入/排放常年 <5–10%;排放对利用率不敏感;硬件 ROI 用代币计价;燃烧铸造均衡里 burn ≪ mint。
- **实例**:Helium(2021–22 峰值——BME 设计本身健全,致命的是比率)、Filecoin(常年个位数利用率;也是 #7)、Hivemapper 类网络。
- **解药**:需求门控排放(利用率乘子、分区上限);BME 铸造下限绑定燃烧(mint ≤ k·burn);硬件 ROI 用服务收入计价;公开收入/排放比。

> **评分说明**:S13/S14/S15 报告在**独立的安全面板**上(每个控制面 0/1/2),绝不并入 54 分螺旋分——攻击风险与螺旋风险正交,且 54 分评分卡为前瞻登记册可比性冻结在 v2。对上述攻击回测:8 个被攻破案例的利润不等式在事前全部可计算([data/security_panel.py](data/security_panel.py) + 成本-奖池图)。
...
| 11 | TVL 靠积分/空投租来,有机占比低 | #11 | ×2 | |
| 12 | 衍生品被递归循环加杠杆,平仓 > 深度 | #12 | ×2 | |

**解读(加权满分 54):**
- **0–7**:结构性死亡螺旋风险低(仍可能有市场风险)。
- **8–18**:存在反身性元素,需明确临界条件与熔断;若"结构"行打到 2,警惕阴跌/去杠杆风险——做归零测试。
- **19–33**:高危,具备一个以上 λ>1 引擎,牛市掩盖、熊市暴露。
- **≥34**:典型死亡螺旋模型,历史同类几乎全灭。

**使用方式**:任意打到 2 的项,直接回到对应 Skill 的"解药"重新设计;权重 ×3 的项命中 = 红线,优先级最高。

> **v2 可操作版**:skill pack 中的 [`scorecard.md`](skills/tokenomics-soundcheck/references/scorecard.md) 已升级为 12 行(含 #11 #12)、满分 54 的可测量版本——每行给出公式、数据源与 0/1/2 判定阈值,并在 18 个历史案例(10 崩盘 + 8 压力幸存者)上完成回测校准:崩盘组 12–37 分,幸存组 1–11 分,无幸存者触发引擎红线(见 `data/scorecard_calibration.py` 与分离度图)。此外已做 **out-of-sample 验证**:15 个泄漏审计的留出案例上判定规则 15/15 正确,并有冻结的前瞻性登记册预注册可证伪预测(见 [`validation/`](validation/README.md))。配套的标准化审计流程见 [`audit-protocol.md`](skills/tokenomics-soundcheck/references/audit-protocol.md),幸存者对照组分析见 [`survivors.md`](skills/tokenomics-soundcheck/references/survivors.md)。
>
> **v4 增补**:除 12 行螺旋分外,新增独立的**安全面板**(S13/S14/S15,腐化成本账本,见 §5b 与 [`economic-security.md`](skills/tokenomics-soundcheck/references/economic-security.md)),审计协议加入了**估值模块**(货币观/股权观分 regime + 风险×估值四象限)与**盲点登记册**,并给出 λ 的雅可比谱半径形式化与反身性 beta 实测方案([`lambda-formalization.md`](skills/tokenomics-soundcheck/references/lambda-formalization.md))。

---

## 7. 给设计者的正向原则(反模式的镜像)

把 15 个 Skill 翻成"应该怎么做",得到健康代币经济的设计公理(1–12 螺旋;13–15 经济安全):
...
11. **租增长必须带归属期**:奖励流量(真实付费)而非存量(停泊 TVL);知道并公开自己的有机基线;空投规模不得压垮 TGE 真实需求。
12. **限制递归**:在自身代币/衍生品上叠杠杆循环是系统性引信;对相关抵押品设 LTV 与供应上限,用真实深度压测平仓。
13. **让腐化成本始终高于奖池**:投票锁定 + 真实时间锁 + 按可借流通量设定的法定人数下限;国库不用原生代币(§5b #13)。
14. **杠杆按操纵成本设定**:LTV、上限与上币都由"预言机窗口内深度"的数学决定,而非资产类别的直觉(§5b #14)。
15. **供给补贴按需求门控**:为利用率而非产能付费;公开收入/排放;有 BME 时 mint ≤ k·burn(§5b #15)。

> 这些公理的正向展开——含完整设计流程、参数基准表与上线清单——见 skill pack 中的 [`design-playbook.md`](skills/tokenomics-soundcheck/references/design-playbook.md)。
...
| #11 雇佣兵积分 | 库外 2023–26 案例:Blast、friend.tech、LRT 积分潮 |
| #12 递归杠杆循环 | 库外案例:stETH 循环平仓(2022.6)、UST Degenbox、ezETH(2024.4) |
| #13 可捕获治理 | 库外案例:Beanstalk(2022.4)、Build Finance、Tornado 治理、Steem |
| #14 可操纵预言机杠杆 | 库外案例:Mango(2022.10)、Venus/XVS、Inverse、Moola |
| #15 供给侧补贴错配 | 库外案例:Helium、Filecoin(亦 #7)、Hivemapper 类 |
